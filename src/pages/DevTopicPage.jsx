...
const DevTopicPage = () => {
  ...
  const topic = getDevTopicById(topicId);
  const {
    setActiveTrack,
    toggleTopicComplete,
    isTopicComplete,
  ...
  // Navigation
  const phaseTopics = topic ? getDevPhaseTopics(topic.phase) : [];
    ...
    if (!complete) {
      ...
      if (allDone) {
        confetti({
          ...
          spread: 70,
          origin: { y: 0.6 },
          colors: ["#8b5cf6", "#d946ef", "#ec4899", "#10b981", "#f59e0b"],
        });
      }
    }
  };
  ...
  if (!topic) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/dev" className="text-violet-400 mt-2 inline-block">
          ← Back to roadmap
        </Link>
      </div>
    );
  ...
  const phaseInfo = getDevPhaseInfo(topic.phase);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-sm text-surface-400 mb-6 flex-wrap">
        <Link to="/dev" className="hover:text-surface-200 transition-colors">
          Dev Home
        </Link>
        <span className="text-surface-600">/</span>
        <Link
          to={`/dev/phase/${topic.phase}`}
      ...
      >
        <div
          ...
          style={{
            background: `linear-gradient(135deg, ${phaseInfo?.gradientFrom || "#8b5cf6"}, ${phaseInfo?.gradientTo || "#d946ef"})`,
          }}
        />
          ...
          <div className="flex items-start justify-between gap-4 mb-5">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                <span
                  className={`text-xs px-2.5 py-1 rounded-lg font-medium ${getDifficultyStyle(topic.difficulty)}`}
                >
                  {topic.difficulty}
                </span>
                {complete && (
              ...
              <div className="flex items-center gap-4 text-sm text-surface-400 flex-wrap">
                <span className="flex items-center gap-1.5">
                  <FaClock className="text-xs text-violet-400" />{" "}
                  {topic.estimatedHours} hours
                </span>
                <span className="flex items-center gap-1.5">
                  <FaListUl className="text-xs text-fuchsia-400" />{" "}
                  {topic.subtopics.length} subtopics
                </span>
                <span className="flex items-center gap-1.5">
                  <FaPlay className="text-xs text-red-400" />{" "}
                  {topic.youtubeVideos.length} videos
                </span>
                {topic.codeExamples && topic.codeExamples.length > 0 && (
                  <span className="flex items-center gap-1.5">
                    <FaCode className="text-xs text-cyan-400" />{" "}
                    {topic.codeExamples.length} code example
                    {topic.codeExamples.length > 1 ? "s" : ""}
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="mb-5">
            <div className="flex items-center justify-between mb-2 text-sm">
              <span className="text-surface-400">
                ...
                <span className="text-surface-200 font-medium">
                  {completedSubCount}/{topic.subtopics.length}
                </span>
              </span>
            ...
            <div className="w-full h-2 bg-surface-700/60 rounded-full overflow-hidden">
              <motion.div
                ...
                style={{
                  background: `linear-gradient(90deg, ${phaseInfo?.gradientFrom || "#8b5cf6"}, ${phaseInfo?.gradientTo || "#d946ef"})`,
                }}
                initial={{ width: 0 }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        >
          <h2 className="text-lg font-bold mb-3 flex items-center gap-2">
            <span className="w-8 h-8 rounded-lg bg-violet-500/10 flex items-center justify-center text-sm">
              📝
            </span>
        ...
        <motion.section
          ...
          style={{ borderLeftColor: phaseInfo?.color || "#8b5cf6" }}
        >
          <h2 className="text-lg font-bold mb-3 flex items-center gap-2">
            <span className="w-8 h-8 rounded-lg bg-fuchsia-500/10 flex items-center justify-center text-sm">
              🎯
            </span>
      ...
      {topic.codeExamples && topic.codeExamples.length > 0 && (
        ...
        >
          <h2 className="text-lg font-bold mb-5 flex items-center gap-2">
            <span className="w-8 h-8 rounded-lg bg-cyan-500/10 flex items-center justify-center text-sm">
              💻
            </span>
        ...
        <div className="grid gap-2">
          {topic.subtopics.map((subtopic, idx) => (
            <InteractiveSubtopic
              ...
              subtopic={subtopic}
              index={idx}
              phaseColor={phaseInfo?.color || "#8b5cf6"}
            />
          ))}
        </div>
      </motion.section>
      ...
      >
        <h2 className="text-lg font-bold mb-5 flex items-center gap-2">
          <span className="w-8 h-8 rounded-lg bg-red-500/10 flex items-center justify-center text-sm">
            🎥
          </span>
                  ...
                  >
                    ...
                    <div className="p-4 border-t border-white/5">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-sm font-semibold hover:text-violet-400 transition-colors inline-flex items-center gap-2"
                      >
                        {video.title}
      ...
      >
        <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
          <span className="w-8 h-8 rounded-lg bg-blue-500/10 flex items-center justify-center text-sm">
            🔗
          </span>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {topic.references.map((ref, idx) => (
            ...
            >
              <div className="w-10 h-10 rounded-xl bg-violet-500/10 flex items-center justify-center text-violet-400 flex-shrink-0 group-hover:bg-violet-500/20 transition-colors">
        ...
        >
          ...
          <div className="flex flex-wrap gap-2">
            {topic.prerequisites.map((preId) => {
              const preTopic = getDevTopicById(preId);
      ...
      >
        {prevTopic ? (
          ...
          >
            ...
            <p className="text-sm font-semibold group-hover:text-violet-400 transition-colors truncate">
              {prevTopic.title}
            </p>
          </Link>
        ) : (
